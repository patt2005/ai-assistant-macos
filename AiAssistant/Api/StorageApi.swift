...
final class StorageApi {
    ...
    func uploadImage(image: NSImage) async throws -> String {
        ...
        guard let imageData = image.tiffRepresentation else {
            throw NSError(domain: "ImageConversionError", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to convert NSImage to Data"])
        }
        ...
        return "\(baseUrl)/api/file/get-file?filename=\(filename)"
    }
}
